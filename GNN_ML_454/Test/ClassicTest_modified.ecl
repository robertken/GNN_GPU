...
instancesPerSlice := COUNT(trainX[1].densedata)/featureCount;
...
ldef0 := ['''layers.Dense(1024, activation='tanh', input_shape=(500,))''',
          '''layers.Dense(1024, activation='relu')''',
          '''layers.Dense(1, activation=None)'''];
					
ldef := [	'''layers.Conv2D(32, kernel_size=(3, 3), activation="relu", input_shape=(32,32,1))''',
...
//These 3 lines need fixing for TF2-----------------------------------------------------------------------------

// GetSession must be called before any other functions
s := GNNI.GetSession(numOfGPUs);
...
//#WORKUNIT('name','2 GPU 2 Thor Fit only - Classic Test Conv1D'); 
