...
EXPORT Keras := MODULE
  ...
  EXPORT STREAMED DATASET(kString) Init(STREAMED DATASET(initParms) initdata, INTEGER numPhysicalComps) :=
    ...
    def initGlobals():
      import numpy as np
      import math, time
      global nodeId, nNodes, maxSliceLen, experimentStartTime
      ...
      import os
      #this next line will force CPU training on a GPU system. CPU-only seems to persist
      #until the interpreter is restarted. HPCC 7.10 should have this ability
      #os.environ["CUDA_VISIBLE_DEVICES"]=str(math.floor(int(nodeId)/numPhysicalComps)) #then mod number of gpus?
      #os.environ["CUDA_VISIBLE_DEVICES"]=str(nodeId)
      ...
      if xAL and yAL and xAL[0].size > 0 and yAL[0].size > 0:
        ...
        with tfSession.as_default():
          with tfSession.graph.as_default():
            ...
            #myfile.write("xAL Shape: "+str(xAL[0].shape))
            #myfile.write("\nyAL Shape: "+str(yAL[0].shape))
            #myfile.write("\nMini Batch Size: " + str(miniBatch))
            #myfile.close()
  ...
  EXPORT STREAMED DATASET(t_Tensor) FitBatchNCCL(
              STREAMED DATASET(t_Tensor) weights,
              STREAMED DATASET(t_Tensor) x,
              STREAMED DATASET(t_Tensor) y,
              UNSIGNED4 seqId,
